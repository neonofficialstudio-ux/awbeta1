
import { getRepository } from "../database/repository.factory";
import type { LedgerEntry, CurrencyType, TransactionType, TransactionSource, CoinTransaction } from "../../types/economy";
import { saveToStorage, loadFromStorage } from "../../api/persist/localStorage";
import type { User, Mission } from "../../types";
import { PLAN_MULTIPLIERS } from "./economy-constants";
import { createNotification } from "../helpers";

const repo = getRepository();
const LEDGER_STORAGE_KEY = "aw_economy_ledger_v6_5";

export const LedgerEngine = {
  /**
   * Records a transaction in the immutable ledger.
   */
  recordTransaction: (
    userId: string,
    currency: CurrencyType,
    amount: number,
    transactionType: TransactionType,
    source: TransactionSource,
    description: string,
    currentBalance: number, // Balance BEFORE transaction + amount
    metadata: any = {}
  ): LedgerEntry => {
    const entry: LedgerEntry = {
      id: `led-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      userId,
      type: currency,
      amount,
      transactionType,
      source,
      timestamp: Date.now(),
      balanceAfter: currentBalance + amount, // Calculate final balance snapshot
      description,
      metadata
    };

    // Persist internal Ledger
    const ledger = LedgerEngine.loadLedger();
    ledger.push(entry);
    LedgerEngine.saveLedger(ledger);

    // If it's a COIN transaction, also sync with the legacy "transactions" table for UI compatibility
    if (currency === 'COIN') {
        const coinTx: CoinTransaction = {
            id: entry.id,
            userId: entry.userId,
            amount: entry.amount,
            type: entry.transactionType,
            source: entry.source,
            description: entry.description,
            date: new Date(entry.timestamp).toLocaleString('pt-BR'),
            dateISO: new Date(entry.timestamp).toISOString()
        };
        repo.insert("transactions", coinTx);
    }

    return entry;
  },

  getLedgerHistory: (userId: string): LedgerEntry[] => {
    const ledger = LedgerEngine.loadLedger();
    return ledger.filter(e => e.userId === userId).sort((a, b) => b.timestamp - a.timestamp);
  },

  /**
   * Checks if a specific transaction type exists for a user, optionally matching a unique ID in metadata.
   * Used for Idempotency checks (e.g. Jackpot Round ID).
   */
  hasTransactionOfType: (userId: string, source: TransactionSource, uniqueId?: string): boolean => {
      const ledger = LedgerEngine.loadLedger();
      return ledger.some(t => 
          t.userId === userId && 
          t.source === source && 
          (uniqueId ? t.metadata?.uniqueId === uniqueId : true)
      );
  },

  clearLedger: () => {
      saveToStorage(LEDGER_STORAGE_KEY, []);
  },

  // Internal helpers
  loadLedger: (): LedgerEntry[] => {
      return loadFromStorage<LedgerEntry[]>(LEDGER_STORAGE_KEY, []);
  },

  saveLedger: (ledger: LedgerEntry[]) => {
      saveToStorage(LEDGER_STORAGE_KEY, ledger);
  }
};

/**
 * Reverts rewards given for a mission.
 * Calculates exact amount based on user plan multiplier and logs the debit.
 */
export const revertMissionRewards = (user: User, mission: Mission) => {
    const multiplier = PLAN_MULTIPLIERS[user.plan] || 1.0;
    const xpToRevert = Math.floor(mission.xp * multiplier);
    const coinsToRevert = Math.floor(mission.coins * multiplier);

    // Create new state (prevent negative balance)
    const updatedUser = {
        ...user,
        xp: Math.max(0, user.xp - xpToRevert),
        coins: Math.max(0, user.coins - coinsToRevert)
    };

    // Log Reversal
    if (coinsToRevert > 0) {
        LedgerEngine.recordTransaction(
            user.id,
            'COIN',
            -coinsToRevert,
            'spend',
            'manual_refund', 
            `Reversão: ${mission.title}`,
            updatedUser.coins
        );
    }
    
    if (xpToRevert > 0) {
         LedgerEngine.recordTransaction(
            user.id,
            'XP',
            -xpToRevert,
            'spend',
            'manual_refund', 
            `Reversão XP: ${mission.title}`,
            updatedUser.xp
        );
    }

    const notifications = [
        createNotification(user.id, 'Recompensas Revertidas', `A missão "${mission.title}" foi revertida. -${xpToRevert} XP, -${coinsToRevert} Coins.`)
    ];

    return { updatedUser, notifications };
};
