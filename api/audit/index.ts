 ...
 * 1. Import the audit engine: `const audit = await import('./api/audit');`
 * 2. Fetch the current data: `const data = await (await import('./api/index')).fetchAdminData();`
 * 3. Run audit for all users: `const results = audit.runAuditForAllUsers(data.allUsers, { transactions: data.allTransactions, submissions: data.missionSubmissions, redeemedItems: data.redeemedItems });`
 * 4. Generate a summary: `const summary = audit.summarizeAudit(results);`
