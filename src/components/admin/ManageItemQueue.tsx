...
import type { UsableItemQueueEntry, ProcessedUsableItemQueueEntry } from '../../types';

interface ManageItemQueueProps {
  queue: UsableItemQueueEntry[];
  onProcess: (queueId: string) => void;
  history: ProcessedUsableItemQueueEntry[];
}

const ManageItemQueue: React.FC<ManageItemQueueProps> = ({ queue, onProcess, history }) => {
  return (
    <div className="space-y-8">
      <div className="bg-[#121212] p-6 rounded-xl border border-gray-800">
        <h3 className="text-xl font-bold mb-6">Fila de Itens Utilizáveis ({queue.length})</h3>
          ...
          <table className="w-full text-sm text-left text-gray-400">
            <thead className="text-xs text-gray-300 uppercase bg-gray-800/50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Link do Post</th>
              ...
              {queue.length > 0 ? (
                  queue.map((item, index) => (
                      <tr key={item.id} className="bg-[#181818] border-b border-gray-800 hover:bg-gray-800/50">
                          ...
                          <td className="px-6 py-4">
                              <a href={item.postUrl} target="_blank" rel="noopener noreferrer" className="text-goldenYellow-400 hover:underline truncate block max-w-xs">
                                  {item.postUrl}
                              </a>
                          </td>
                  ...
                  <tr><td colSpan={6} className="text-center py-8">A fila está vazia.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-[#121212] p-6 rounded-xl border border-gray-800">
        <h3 className="text-xl font-bold mb-6">Histórico de Itens Concluídos</h3>
          ...
          <table className="w-full text-sm text-left text-gray-400">
            <thead className="text-xs text-gray-300 uppercase bg-gray-800/50 sticky top-0">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Item</th>
                <th scope="col" className="px-6 py-3">Link do Post</th>
                <th scope="col" className="px-6 py-3">Concluído em</th>
              </tr>
            </thead>
            <tbody>
              {history.length > 0 ? (
                  ...
                  <tr><td colSpan={4} className="text-center py-8">Nenhum item foi concluído ainda.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
export default ManageItemQueue;